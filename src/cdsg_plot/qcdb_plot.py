...
def bars(data, title="", saveas=None, relpath=False, graphicsformat=["pdf"], view=True):
    ...
    *data*. The plot is labeled with *title* and each bar with *mc* key and
    ...
    pltuid = (
        ...
        + "_"
        + hashlib.sha1(
            (title + repr([bar["mc"] for bar in data if bar is not None])).encode()
        ).hexdigest()
    )
...
def disthist(
    ...
    *xmin* to *xmax*, x-axis label *xtitle* and overall title *title*.
    ...
    pltuid = (
        ...
        + "_"
        + hashlib.sha1(
            (title + str(me) + str(stde) + str(xmin) + str(xmax)).encode()
        ).hexdigest()
    )
    ...
    *color* is None, slats are black, if 'sapt', colors are taken from *color*
    ...
    for rxn in data:
        ...
        if Nweft == 1:
            ...
            ax.plot(slat, posnS, color=clr, linewidth=1.0, solid_capstyle="round")
        ax.plot(
            thread, posnT, color=clr, linewidth=0.5, solid_capstyle="round", alpha=0.3
        )
        ...
        if not (mousetext or mouselink or mouseimag):
            if labeled and len(data) < 200:
                try:
                    toplblposn = next(item for item in xvals if item is not None)
                    botlblposn = next(
                        item for item in reversed(xvals) if item is not None
                    )
                except StopIteration:
                    pass
                else:
    ...
    pltuid = (
        title
        + "_"
        + ("lbld" if labeled else "bare")
        + "_"
        + hashlib.sha1((title + repr(labels) + repr(xlimit)).encode()).hexdigest()
    )
...
def ternary(
    ...
    if labeled:
        ...
        pathPos = Path([(0.0, 0.0), (1.0, 0.0), (0.5, 0.866), (0.0, 0.0)], codes)
        pathNeg = Path([(0.0, 0.0), (-0.5, 0.866), (0.5, 0.866), (0.0, 0.0)], codes)
        ...
        ax.plot([0.667, 0.5], [0.0, 0.866], color="#eeb4b4", lw=0.5)
        ax.plot([-0.333, 0.5], [0.577, 0.866], color="#eeb4b4", lw=0.5)
        ax.plot([0.333, 0.5], [0.0, 0.866], color="#7ec0ee", lw=0.5)
    ...
    pltuid = (
        ...
        + ("lbld" if labeled else "bare")
        + "_"
        + hashlib.sha1((title + repr(sapt)).encode()).hexdigest()
    )
...
def iowa(
    ...
    *labels*.
    ...
    axt.set_xticklabels(
        aa, minor=False, rotation=60, size="small"
    )  # comment for quick nolabel, whiteback
    axt.set_yticklabels(
        aa, minor=False, size="small"
    )  # comment for quick nolabel, whiteback
...
if __name__ == "__main__":
    ...
    flat(
        [
            {"sys": "1", "color": 0.6933450559423702, "data": [0.45730000000000004]},
            {"sys": "2", "color": 0.7627027688599753, "data": [0.6231999999999998]},
            {"sys": "3", "color": 0.7579958735528617, "data": [2.7624999999999993]},
            {"sys": "4", "color": 0.7560883254421639, "data": [2.108600000000001]},
            {"sys": "5", "color": 0.7515161912065955, "data": [2.2304999999999993]},
            {"sys": "6", "color": 0.7235223893438876, "data": [1.3782000000000014]},
            {"sys": "7", "color": 0.7120099024225569, "data": [1.9519000000000002]},
            {"sys": "8", "color": 0.13721565059144678, "data": [0.13670000000000004]},
            {"sys": "9", "color": 0.3087395095814767, "data": [0.2966]},
            {
                "sys": "10",
                "color": 0.25493207637105103,
                "data": [-0.020199999999999996],
            },
            {"sys": "11", "color": 0.24093814608979347, "data": [-1.5949999999999998]},
            {"sys": "12", "color": 0.3304746631959777, "data": [-1.7422000000000004]},
            {"sys": "13", "color": 0.4156050644764822, "data": [0.0011999999999989797]},
            {"sys": "14", "color": 0.2667207259626991, "data": [-2.6083999999999996]},
            {"sys": "15", "color": 0.3767053567641695, "data": [-1.5090000000000003]},
            {"sys": "16", "color": 0.5572641509433963, "data": [0.10749999999999993]},
            {"sys": "17", "color": 0.4788598239641578, "data": [0.29669999999999996]},
            {"sys": "18", "color": 0.3799031371351281, "data": [0.10209999999999964]},
            {"sys": "19", "color": 0.5053227185999078, "data": [0.16610000000000014]},
            {"sys": "20", "color": 0.2967660584483015, "data": [-0.37739999999999974]},
            {"sys": "21", "color": 0.38836460733750316, "data": [-0.4712000000000005]},
            {"sys": "22", "color": 0.5585849893078809, "data": [0.30460000000000065]},
            {"sys": "BzBz_PD36-1.8", "color": 0.1383351040559965, "data": [-1.1921]},
            {"sys": "BzBz_PD34-2.0", "color": 0.23086034843049832, "data": [-1.367]},
            {
                "sys": "BzBz_T-5.2",
                "color": 0.254318060864096,
                "data": [-0.32230000000000025],
            },
            {"sys": "BzBz_T-5.1", "color": 0.26598486566733337, "data": [-0.3428]},
            {
                "sys": "BzBz_T-5.0",
            ...
            {"sys": "PyPy_S2-3.8", "color": 0.1690757103699542, "data": [-1.0932]},
            {"sys": "PyPy_S2-3.5", "color": 0.25615734567417053, "data": [-1.4617]},
            {
                "sys": "PyPy_S2-3.7",
                "color": 0.19566550224566906,
                "data": [-1.2103999999999995],
            },
            {"sys": "PyPy_S2-3.6", "color": 0.22476748600170826, "data": [-1.3333]},
            {"sys": "BzBz_PD32-2.0", "color": 0.31605681987208084, "data": [-1.6637]},
            {
                "sys": "BzBz_T-4.8",
            ...
            {"sys": "BzBz_PD36-1.9", "color": 0.14128552184232473, "data": [-1.1517]},
            {"sys": "BzBz_S-3.7", "color": 0.08862098445220466, "data": [-1.3414]},
            {
                "sys": "BzH2S-4.0",
                "color": 0.33637540012259076,
                "data": [-0.2265999999999999],
            },
            {"sys": "BzBz_PD36-1.5", "color": 0.13203548045236127, "data": [-1.3035]},
            {"sys": "BzBz_S-3.8", "color": 0.0335358832178858, "data": [-1.2022]},
            {"sys": "BzBz_S-3.9", "color": 0.021704594689389095, "data": [-1.0747]},
            {
                "sys": "PyPy_T3-5.1",
            ...
            {"sys": "BzBz_PD34-1.8", "color": 0.22685419834431494, "data": [-1.476]},
            {
                "sys": "BzBz_PD34-1.9",
            ...
            {"sys": "FaOOFaON-4.0", "color": 0.7589275618320565, "data": [2.0586]},
            {
                "sys": "FaOOFaON-3.7",
            ...
            {"sys": "FaNNFaNN-3.9", "color": 0.7548200901251662, "data": [2.2089]},
            {
                "sys": "FaONFaON-3.8",
            ...
            {"sys": "1", "color": 0.2784121805328983, "data": [0.3508]},
            {"sys": "2", "color": 0.22013842798900166, "data": [-0.034600000000000186]},
            {"sys": "3", "color": 0.12832496088281312, "data": [0.20360000000000023]},
            {"sys": "4", "color": 0.6993695033529733, "data": [1.9092000000000002]},
            {"sys": "5", "color": 0.7371192790053749, "data": [1.656600000000001]},
            {"sys": "6", "color": 0.5367033190796172, "data": [0.27970000000000006]},
            {"sys": "7", "color": 0.3014220615964802, "data": [0.32289999999999974]},
            {"sys": "8", "color": 0.01605867807629261, "data": [0.12199999999999994]},
            {"sys": "9", "color": 0.6106300539083558, "data": [0.3075999999999999]},
            {"sys": "10", "color": 0.6146680031333968, "data": [0.6436000000000002]},
            {"sys": "11", "color": 0.6139747851721759, "data": [0.4551999999999996]},
            {"sys": "12", "color": 0.32122739401126593, "data": [0.44260000000000005]},
            {"sys": "13", "color": 0.24678148099136055, "data": [-0.11789999999999967]},
            {"sys": "14", "color": 0.23700950710597016, "data": [0.42689999999999995]},
            {"sys": "15", "color": 0.23103396678138563, "data": [0.3266]},
            {"sys": "16", "color": 0.1922070769654413, "data": [0.0696000000000001]},
            {"sys": "17", "color": 0.19082151944747366, "data": [0.11159999999999992]},
            {"sys": "18", "color": 0.2886200282444196, "data": [0.4114]},
            {"sys": "19", "color": 0.23560171133945224, "data": [-0.1392]},
            {"sys": "20", "color": 0.3268270751294533, "data": [0.5593]},
            {"sys": "21", "color": 0.7324460869158442, "data": [0.6806000000000001]},
        ],
        color="sapt",
        title="MP2-CP-adz",
        mae=1.21356003247,
